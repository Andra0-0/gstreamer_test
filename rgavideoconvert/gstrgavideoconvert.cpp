...
// /*Debug*/static gboolean report_once = FALSE;
static gboolean gst_rga_info_from_video_frame(rga_info_t *info,
  ...
  // /*Debug*/if (report_once) {
  //   GST_INFO("RGA Convert: format=%d, width=%d, height=%d, hstride=%d, vstride=%d",
  //            rga_format, width, height, hstride, vstride);
  //   report_once = FALSE;
  // }
...
static void gst_rga_video_convert_init(GstRgaVideoConvert *rgavideoconvert) {
  GST_DEBUG_OBJECT(rgavideoconvert, "init");
}
