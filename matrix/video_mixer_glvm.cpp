...
gint VideoMixerGlvm::init()
{
  ...
  do {
    ...
    //           pad, GST_PAD_PROBE_TYPE_BUFFER, deffunc_videoframe_info);
...
gint VideoMixerGlvm::deinit()
{
  // TODO gst_element_factory_make failed?
}

// gint VideoMixerGlvm::connect(VideomixConfig &cfg)
// {
//   gint ret = -1;
...
//       ALOGD("VideoMixerGlvm connect %s error, already exist connect", GST_PAD_NAME(cfg.src_pad_));
//       break;
//     }
...
//       ALOGD("VideoMixerGlvm failed to link pad");
//       break;
//     }
...
gint VideoMixerGlvm::connect(VideomixConfig &cfg)
{
  ...
  do {
    ...
    if (!gst_element_link_many(queue, glupload, glconvert, NULL)) {
      ALOGD("VideoMixerGlvm failed to link elements");
      break;
    }
...
gint VideoMixerGlvm::disconnect(GstPad *src_pad)
{
  do {
...
gint VideoMixerGlvm::disconnect_all()
{
  return 0;
}

string VideoMixerGlvm::get_info()
{
  std::ostringstream oss;
  GstState state, pending;

  oss << "\n\t============ get_info ============\n"
      << "\tClass: VideoMixerGlvm, bin name: " << GST_OBJECT_NAME(bin_) << "\n";
...
GstPad* VideoMixerGlvm::get_request_pad(const gchar *name)
{
  GstPad *pad = nullptr;
  ...
  do {
    if (g_str_has_prefix(name, "video_sink_")) {
      ...
      if (sink_pad) {
        ...
        ALOGD("VideoMixerGlvm failed to get sink pad");
      }
      ...
      if (queue_src_pad) {
        ...
        ALOGD("VideoMixerGlvm failed to get queue src pad");
      }
    }
  } while(0);
